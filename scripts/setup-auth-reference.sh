...
echo "🔗 OneMount Authentication Reference Setup"
echo "=========================================="
echo ""

# Find the newest/best auth token file
echo "🔍 Finding canonical authentication token location..."
...
echo "✅ Canonical token location: $CANONICAL_TOKEN"
...
if command -v jq >/dev/null 2>&1; then
    ...
    if [[ "$expires_at" != "0" ]]; then
        ...
        echo "⚠️  No expiration info in token"
    fi
else
    echo "⚠️  Cannot validate token (jq not available)"
fi
...
ONEMOUNT_AUTH_TOKEN_PATH=$CANONICAL_TOKEN
...
export ONEMOUNT_AUTH_TOKEN_PATH="$CANONICAL_TOKEN"
...
echo "🧪 Testing authentication reference..."

# Source the environment file
source "$ENV_FILE"

echo "   Canonical path: $ONEMOUNT_AUTH_TOKEN_PATH"
echo "   Docker path: $ONEMOUNT_AUTH_PATH_DOCKER"
echo "   File exists: $(test -f "$ONEMOUNT_AUTH_TOKEN_PATH" && echo "✅ YES" || echo "❌ NO")"

echo ""
echo "🎉 Authentication reference setup complete!"
echo ""
echo "📋 Usage:"
...
echo ""
echo "💡 The tokens are now referenced directly - no copying or symlinking!"
