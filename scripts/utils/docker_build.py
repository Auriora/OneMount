...
class DockerPackageBuilder:
    ...
    DEPRECATED: This class is kept for reference but is no longer used.
    The canonical build process now uses scripts/build-deb-package.sh
    which is called by build_debian_package_docker() function below.
    
    This approach provides:
    - Single source of truth for build logic
    - Easier maintenance and debugging
    - Consistency between CLI and direct script usage
    - Proper webkit build tag support
    """
...
def build_debian_package_docker(verbose: bool = False, clean: bool = False, force_rebuild_image: bool = False) -> bool:
    ...
    This function now calls the canonical shell script (scripts/build-deb-package.sh)
    instead of reimplementing the build logic in Python. This ensures consistency
