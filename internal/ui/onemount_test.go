...
func TestGetAccountName(t *testing.T) {
	...
	// Create a mock auth tokens file if it doesn't already exist
	// (CI runners will not have this file yet)
	if _, err := os.Stat(validTokensPath); err != nil {
		// Create a mock auth tokens file with valid JSON
		mockTokens := []byte(`{"Account":"test@example.com","AccessToken":"test-token","RefreshToken":"test-refresh","ExpiresAt":1234567890}`)
		err = os.WriteFile(validTokensPath, mockTokens, 0644)
		require.NoError(t, err, "Failed to create mock auth tokens file")
	}
